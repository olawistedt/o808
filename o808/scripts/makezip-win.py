...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\o808 Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\o808 Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\\installer\\changelog.txt",
      projectpath + "\\installer\\known-issues.txt",
      projectpath + "\\manual\\o808 manual.pdf" 
    ]
  else:
    files = [
      projectpath + "\\build-win\\o808.vst3\\Contents\\x86_64-win\\o808.vst3",
      projectpath + "\\build-win\\o808_x64.exe"  
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\o808-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\o808-app_x64.pdb"  
  ]
